...
from app.src.algorithm.base.sadtalker.src.face3d.extract_kp_videos_safe import KeypointExtractor
...
class Preprocesser:
    def __init__(self, gfpgan_path, device='cuda'):
        self.predictor = KeypointExtractor(gfpgan_path, device)
    ...
    def crop(self, img_np_list, still=False, xsize=512):    # first frame for all video
        ...
        if lm is None:
            raise AttributeError('can not detect the landmark from source image')
